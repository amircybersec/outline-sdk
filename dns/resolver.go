...
func queryDatagram(conn io.ReadWriter, q dnsmessage.Question) (*dnsmessage.Message, error) {
	...
	for {
		...
		if err != nil {
			return nil, &nestedError{ErrReceive, wrapErrors(returnErr, fmt.Errorf("read message failed: %w", err))}
		}
		var msg dnsmessage.Message
		if err := msg.Unpack(buf[:n]); err != nil {
			returnErr = wrapErrors(returnErr, err)
...
func wrapErrors(err1, err2 error) error {
	...
	case err2 == nil:
		return err1
	default:
		return fmt.Errorf("%v: %w", err2, err1)
	}
}
